...
UBYTE actor_attached;  // Keeps track of whether the player is currently on an actor and inheriting
                       // its movement
...
WORD jump_reduction;  // Holds the reduction amount that has been normalized over the number of jump
                      // frames
WORD boost_val;
...
// UBYTE grounded;             //Variable to keep compatability with other plugins that use the
// older 'grounded' check
...
void platform_init(void) BANKED {
  ...
  // This ensures that, by itself, the plat run boost active on any single frame cannot overflow a
  // WORD. It is complemented by another check in the jump itself that works with the actual
  // velocity.
  if (plat_run_boost != 0) {
  ...
  jump_per_frame = plat_jump_vel /
                   MIN(15, plat_hold_jump_max);  // jump force applied per frame in the JUMP_STATE
  jump_reduction = plat_jump_reduction /
                   plat_hold_jump_max;  // Amount to reduce subequent jumps per frame in JUMP_STATE
  dash_dist = plat_dash_dist / plat_dash_frames;  // Dash distance per frame in the DASH_STATE
  boost_val = plat_run_boost /
              plat_hold_jump_max;  // Vertical boost from horizontal speed per frame in JUMP STATE
...
void platform_update(void) BANKED {
  ...
  switch (plat_state) {
    ...
    case FALL_STATE: {
      jump_type = 0;  // Keep this here, rather than in init, so that we can easily track float as a
                      // jump type
    ...
    case GROUND_STATE: {
      ...
      if (actor_attached) {
        ...
        // We're setting these to the platform's position, rather than the actor so that if
        // something causes the player to detach (like hitting the roof), they won't automatically
        // get re-attached in the subsequent actor collision step.
        mp_last_y = last_actor->pos.y;
        temp_y = last_actor->pos.y;
      } else if (nocollide != 0) {
        // If we're dropping through a platform
        pl_vel_y = 7000;  // magic number, rough minimum for actually having the player descend
                          // through a platform
        temp_y = PLAYER.pos.y;
      } else {
        // Normal gravity
      ...
      //================================================================================================================
    case JUMP_INIT:
      // Right now this has a limited use for triggered jumps because many of the jump effects
      // depend on testing INPUT_PLATFORM_JUMP But if the player switches to this state without
    ...
    case JUMP_STATE: {
      ...
      if (hold_jump_val != 0 && INPUT_PLATFORM_JUMP) {
        ...
        // This is a test to see if the results will overflow pl_vel_y. Note, pl_vel_y is negative
        // here.
    ...
    case DASH_INIT: {
      ...
      goto gotoCounters;  // Dash Init has a return, unlike other initialization phases, because its
                          // calculations are time consuming and we don't want to deal with
    ...
    case DASH_STATE: {
      ...
      if (PLAYER.dir == DIR_RIGHT) {
        ...
        // in the future, I should build a reversed version of this section for dashing through
        // walls. However, it's not quite as simple as reversing the direction of the check. The
        // loops need to store the player's width and only return when there are enough spaces in a
        // row
        while (tile_current != tile_x) {
          // Don't go past camera bounds
          if ((plat_camera_block & 2) &&
              tile_current > PX_TO_TILE(camera_x + SCREEN_WIDTH_HALF - 16)) {
          ...
          while (tile_start != tile_end) {
            ...
            // Check for Triggers at each step. If there is a trigger stop the dash (but don't run
            // the trigger yet).
      ...
      endRcol:
        if (plat_dash_momentum == 1 || plat_dash_momentum == 3) {
          // Dashes don't actually use velocity, so we will simulate the momentum by adding the full
          // run speed.
      ...
      if (plat_dash_momentum >= 2) {
        ...
        if (INPUT_PRESSED(INPUT_PLATFORM_JUMP)) {
          ...
          if (ct_val != 0) {
            ...
            actor_attached = FALSE;
            // We can't switch states for jump frames, so approximate the height. Engine val limits
...
gotoXCol: {
  ...
  if (new_x > PX_TO_SUBPX(*edge_right + SCREEN_WIDTH - 16)) {
    ...
    // Same but for left side. This side needs a 1 tile (8px) buffer so it doesn't overflow the
    // variable.
...
gotoYCol: {
  ...
  if (deltaY > 0) {
    ...
    // If previously grounded and gravity is not enough to pull us down to the next tile, manually
    // check it for the next slope This prevents the "animation glitch" when going down slopes
    ...
    while (tile_y <= new_tile_y) {
      ...
      if (IS_ON_SLOPE(col)) {
        if ((col & COLLISION_SLOPE) == COLLISION_SLOPE_45_RIGHT) {
          slope_y_coord =
              PX_TO_SUBPX((TILE_TO_PX(tile_y) + (8 - x_offset) - PLAYER.bounds.bottom)) - 1;
        } else if ((col & COLLISION_SLOPE) == COLLISION_SLOPE_225_RIGHT_BOT) {
          slope_y_coord =
              PX_TO_SUBPX((TILE_TO_PX(tile_y) + (8 - DIV_2(x_offset)) - PLAYER.bounds.bottom)) - 1;
        } else if ((col & COLLISION_SLOPE) == COLLISION_SLOPE_225_RIGHT_TOP) {
          slope_y_coord =
              PX_TO_SUBPX((TILE_TO_PX(tile_y) + (4 - DIV_2(x_offset)) - PLAYER.bounds.bottom)) - 1;
        }
          ...
          slope_y_coord =
              PX_TO_SUBPX((TILE_TO_PX(tile_y) + DIV_2(x_offset) - PLAYER.bounds.bottom + 4)) - 1;
        } else if ((col & COLLISION_SLOPE) == COLLISION_SLOPE_225_LEFT_TOP) {
          slope_y_coord =
              PX_TO_SUBPX((TILE_TO_PX(tile_y) + DIV_2(x_offset) - PLAYER.bounds.bottom)) - 1;
        }
      }
    ...
    if (nocollide == 0) {
      ...
      while (tile_start != tile_end) {
        ...
        land:
          new_y = PX_TO_SUBPX(((TILE_TO_PX(tile_y))-PLAYER.bounds.bottom)) - 1;
          actor_attached = FALSE;  // Detach when MP moves through a solid tile.
          // The distinction here is used so that we can check the velocity when the player hits the
          // ground.
          if (plat_state == GROUND_STATE) {
            que_state = GROUND_STATE;
            pl_vel_y = 256;
...
gotoSwitch2:
  ...
  switch (plat_state) {
    ...
    case JUMP_STATE: {
      ...
      // STATE
      // CHANGE------------------------------------------------------------------------------------------------
      // Above: JUMP-> NEUTRAL when a) player starts descending, b) player hits roof, c) player
      // stops pressing, d)jump frames run out. JUMP -> WALL check
      wall_check();
...
gotoCounters:
  ...
  if (state_events[plat_state].script_addr != 0) {
    script_execute(state_events[plat_state].script_bank, state_events[plat_state].script_addr, 0,
                   0);
  }
}

void basic_anim(void) BANKED {
  // This animation is currently shared by jumping, dashing, and falling. Dashing doesn't need this
  // complexity though. Here velocity overrides direction. Whereas on the ground it is the reverse.
...
void ladder_check(void) BANKED {
  ...
  if (INPUT_UP || INPUT_DOWN) {
    ...
    if (IS_LADDER(tile_at(tile_x_mid, tile_y))) {
      PLAYER.pos.x =
          PX_TO_SUBPX((TILE_TO_PX(tile_x_mid) + 4 - (PLAYER.bounds.left + p_half_width)));
...
void ladder_switch(void) BANKED {
  ...
  // Above is the default GBStudio setup. However it seems worth adding a jump-from-ladder option,
  // at the very least to drop down.
  if (INPUT_PRESSED(INPUT_PLATFORM_JUMP)) {
    que_state = FALL_INIT;
  }
  // Check for final frame
...
void dash_init_switch(void) BANKED {
  WORD new_x;
  // If the player is pressing a direction (but not facing a direction, ie on a wall or on a changed
  // frame)
  if (INPUT_RIGHT) {
    PLAYER.dir = DIR_RIGHT;
  } else if (INPUT_LEFT) {
  ...
  // Set new_x be the final destination of the dash (ie. the distance covered by all of the dash
  // frames combined)
  if (PLAYER.dir == DIR_RIGHT) {
  ...
  if (plat_dash_through == 3 && plat_dash_momentum < 2) {
    dash_end_clear = true;  // Assume that the landing spot is clear, and disable if we collide
                            // below
    UBYTE tile_start = PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.y) + PLAYER.bounds.top);
    UBYTE tile_end = PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.y) + PLAYER.bounds.bottom) + 1;
    ...
    if (PLAYER.dir == DIR_RIGHT) {
      ...
      if (PLAYER.pos.x + PX_TO_SUBPX(PLAYER.bounds.right) + (dash_dist * (plat_dash_frames)) >
        ...
        while (tile_xl !=
               tile_xr) {  // This checks all the tiles between the left bounds and the right bounds
          while (tile_start !=
                 tile_end) {  // This checks all the tiles that the character occupies in height
      ...
      if (PLAYER.pos.x <=
          ((dash_dist * plat_dash_frames) + PX_TO_SUBPX(PLAYER.bounds.left)) + PX_TO_SUBPX(8)) {
        dash_end_clear = false;  // To get around unsigned position, test if the player's current
                                 // position is less than the total dist.
      } else {
        UBYTE tile_xl = PX_TO_TILE(SUBPX_TO_PX(new_x) + PLAYER.bounds.left);
        UBYTE tile_xr = PX_TO_TILE(SUBPX_TO_PX(new_x) + PLAYER.bounds.right) + 1;
