...
UBYTE
plat_run_boost;   // Additional jump height based on player horizontal speed
UBYTE plat_dash;  // Choice of input for dashing: double-tap, interact, or down
                  // and interact
...
void platform_init(void) BANKED {
    ...
    while (32000 - (plat_jump_vel / MIN(15, plat_hold_jump_max)) -
               plat_jump_min <
           0) {
    ...
    if (plat_run_boost != 0) {
        while ((32000 / plat_run_boost) <
               (VEL_TO_SUBPX(plat_run_vel) / plat_hold_jump_max)) {
            plat_run_boost--;
        }
    }
    ...
    jump_per_frame =
        plat_jump_vel / MIN(15, plat_hold_jump_max);  // jump force applied per
                                                      // frame in the JUMP_STATE
    jump_reduction = plat_jump_reduction /
                     plat_hold_jump_max;  // Amount to reduce subequent jumps
                                          // per frame in JUMP_STATE
    dash_dist = plat_dash_dist /
                plat_dash_frames;  // Dash distance per frame in the DASH_STATE
    boost_val =
        plat_run_boost / plat_hold_jump_max;  // Vertical boost from horizontal
...
void platform_update(void) BANKED {
    ...
    UBYTE tile_x_mid = PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.x) +
                                  PLAYER.bounds.left + p_half_width);
    UBYTE tile_y =
        PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.y) + PLAYER.bounds.top + 1);
    ...
    switch (plat_state) {
            ...
            if (actor_attached) {
                ...
                if (last_actor->disabled == TRUE) {
                    ...
                    // from the platform
                           ...
                           last_actor->pos.x + 16 +
                               PX_TO_SUBPX(last_actor->bounds.right)) {
                    que_state = FALL_STATE;
                    actor_attached = FALSE;
                    // If the player is off the platform to the left, detach
                } else if (PLAYER.pos.x + 16 +
                               PX_TO_SUBPX(PLAYER.bounds.right) <
                           last_actor->pos.x +
                               PX_TO_SUBPX(last_actor->bounds.left)) {
                    que_state = FALL_STATE;
                    actor_attached = FALSE;
                } else {
                ...
                pl_vel_y =
                    7000;  // magic number, rough minimum for actually having
        ...
        case DASH_STATE: {
            ...
            UBYTE tile_current;  // For tracking collisions across longer
                                 // distances
            UBYTE tile_start =
                PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.y) + PLAYER.bounds.top);
            UBYTE tile_end =
                PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.y) + PLAYER.bounds.bottom) +
                1;
            col = 0;
            ...
            if (PLAYER.dir == DIR_RIGHT) {
                ...
                tile_current =
                    PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.x) + PLAYER.bounds.right);
                // Get tile x-coord of final position
                UWORD new_x = PLAYER.pos.x + (dash_dist);
                UBYTE tile_x =
                    PX_TO_TILE(SUBPX_TO_PX(new_x) + PLAYER.bounds.right) + 1;
                ...
                while (tile_current != tile_x) {
                    ...
                    if ((plat_camera_block & 2) &&
                        tile_current >
                            PX_TO_TILE(camera_x + SCREEN_WIDTH_HALF - 16)) {
                        new_x = PX_TO_SUBPX(TILE_TO_PX(tile_current) -
                                            PLAYER.bounds.right) -
                                1;
                        dash_currentframe == 0;
                        goto endRcol;
                    ...
                    while (tile_start != tile_end) {
                        ...
                        if (plat_dash_through != 3 || dash_end_clear == FALSE) {
                            if (tile_at(tile_current, tile_start) &
                                COLLISION_LEFT) {
                                // The landing space is the tile we collided on,
                                // but one to the left
                                new_x =
                                    PX_TO_SUBPX((TILE_TO_PX(
                                        tile_current))-PLAYER.bounds.right) -
                                    1;
                                col = 1;
                                last_wall = 1;
                        ...
                        /*if (plat_dash_through < 2){
                            if (trigger_at_tile(tile_current, tile_start) !=
                        NO_TRIGGER_COLLISON) { new_x = ((((tile_current+1) << 3)
                        - PLAYER.bounds.right) << 4);
                            }
                        }*/
                        tile_start++;
                    }

                    tile_start = PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.y) +
                                            PLAYER.bounds.top);
                    tile_current += 1;
                }
            endRcol:
            ...
            // Left Dash Movement & Collision
                ...
                // Get tile x-coord of player position
                tile_current =
                    PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.x) + PLAYER.bounds.left);
                // Get tile x-coord of final position
                WORD new_x = PLAYER.pos.x - (dash_dist);
                UBYTE tile_x =
                    PX_TO_TILE(SUBPX_TO_PX(new_x) + PLAYER.bounds.left) - 1;
                // CHECK EACH SPACE FROM START TO END
                while (tile_current != tile_x) {
                    // Camera lock check
                    if ((plat_camera_block & 1) &&
                        tile_current <
                            PX_TO_TILE(camera_x - SCREEN_WIDTH_HALF)) {
                        new_x = PX_TO_SUBPX(TILE_TO_PX(tile_current + 1) -
                                            PLAYER.bounds.left) +
                                1;
                        dash_currentframe == 0;
                        goto endLcol;
                    ...
                    while (tile_start != tile_end) {
                        // check for walls
                        if (plat_dash_through != 3 ||
                            dash_end_clear ==
                                FALSE) {  // If you collide with walls
                            if (tile_at(tile_current, tile_start) &
                                COLLISION_RIGHT) {
                                new_x =
                                    PX_TO_SUBPX(TILE_TO_PX(tile_current + 1) -
                                                PLAYER.bounds.left) +
                                    1;
                                col = -1;
                                last_wall = -1;
                        ...
                        /*if (plat_dash_through  < 2){
                            if (trigger_at_tile(tile_current, tile_start) !=
                        NO_TRIGGER_COLLISON) { new_x = ((((tile_current - 1) <<
                        3) - PLAYER.bounds.left) << 4); goto endLcol;
                            }
                        }*/
                        tile_start++;
                    }
            ...
            if (plat_dash_momentum >= 2) {
                ...
                UBYTE tile_start =
                    PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.x) + PLAYER.bounds.left);
                UBYTE tile_end = PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.x) +
                                            PLAYER.bounds.right) +
                                 1;
                if (deltaY > 0) {
                    // Moving Downward
                    ...
                    UBYTE tile_y =
                        PX_TO_TILE(SUBPX_TO_PX(new_y) + PLAYER.bounds.bottom);
                    while (tile_start != tile_end) {
                        if (tile_at(tile_start, tile_y) & COLLISION_TOP) {
                            // Land on Floor
                            new_y = PX_TO_SUBPX(TILE_TO_PX(tile_y) -
                    ...
                    WORD new_y = PLAYER.pos.y + deltaY;
                    UBYTE tile_y =
                        PX_TO_TILE(SUBPX_TO_PX(new_y) + PLAYER.bounds.top);
                    while (tile_start != tile_end) {
                        if (tile_at(tile_start, tile_y) & COLLISION_BOTTOM) {
                            new_y = PX_TO_SUBPX((UBYTE)TILE_TO_PX(tile_y + 1) -
                                                PLAYER.bounds.top) +
                                    1;
                            pl_vel_y = 0;
                            break;
                ...
                // Clamp Y Velocity
                pl_vel_y =
                    CLAMP(pl_vel_y, -plat_max_fall_vel, plat_max_fall_vel);
            } else {
                temp_y = PLAYER.pos.y;
            }
        }
            ...
            switch (plat_run_type) {
                case 0:
                    ...
                    if (pl_vel_x < 0 && plat_turn_acc != 0) {
                        ...
                        run_stage = 0;
                        pl_vel_x = CLAMP(pl_vel_x + plat_walk_acc, plat_min_vel,
                                         plat_walk_vel);
                    }
                    pl_vel_x *= dir;
                    deltaX += VEL_TO_SUBPX(pl_vel_x);
                ...
                case 1:
                    ...
                    pl_vel_x = CLAMP(pl_vel_x + plat_run_acc, plat_min_vel,
                                     plat_run_vel);
                    pl_vel_x *= dir;
                    deltaX += VEL_TO_SUBPX(pl_vel_x);
                    run_stage = 1;
...
gotoXCol: {
    ...
    UBYTE tile_start =
        PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.y) + PLAYER.bounds.top);
    UBYTE tile_end =
        PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.y) + PLAYER.bounds.bottom) + 1;
    ...
    if (new_x > PX_TO_SUBPX(*edge_right + SCREEN_WIDTH - 16)) {
        ...
        if (new_x > PLAYER.pos.x) {
            ...
            new_x =
                PLAYER.pos.x -
                MIN(PLAYER.pos.x - PX_TO_SUBPX(*edge_right + SCREEN_WIDTH - 16),
                    16);
        }
        ...
        if (deltaX < 0) {
            ...
            new_x = PLAYER.pos.x +
                    MIN(PX_TO_SUBPX(*edge_left + 8) - PLAYER.pos.x, 16);
        }
    }
    ...
    if (new_x > PLAYER.pos.x) {
        ...
        UBYTE tile_x_mid = PX_TO_TILE(SUBPX_TO_PX(new_x) + PLAYER.bounds.left +
                                      p_half_width + 1);
        ...
        while (tile_start != tile_end) {
            ...
            if (col & COLLISION_LEFT) {
                ...
                new_x =
                    PX_TO_SUBPX(TILE_TO_PX(tile_x) - PLAYER.bounds.right) - 1;
                pl_vel_x = 0;
                col = 1;
                last_wall = 1;
        ...
        while (tile_start != tile_end) {
            ...
            if (col & COLLISION_RIGHT) {
                ...
                new_x =
                    PX_TO_SUBPX((TILE_TO_PX(tile_x + 1)) - PLAYER.bounds.left) +
                    1;
                pl_vel_x = 0;
                col = -1;
                last_wall = -1;
...
gotoYCol: {
    ...
    UBYTE tile_start =
        PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.x) + PLAYER.bounds.left);
    UBYTE tile_end =
        PX_TO_TILE(SUBPX_TO_PX(PLAYER.pos.x) + PLAYER.bounds.right) + 1;
    if (deltaY > 0) {
        // Moving Downward
        WORD new_y = PLAYER.pos.y + deltaY;
        ...
        UWORD x_mid_coord =
            (SUBPX_TO_PX(PLAYER.pos.x) + PLAYER.bounds.left + p_half_width + 1);
        while (tile_y <= new_tile_y) {
            ...
            if (IS_ON_SLOPE(col)) {
                if ((col & COLLISION_SLOPE) == COLLISION_SLOPE_45_RIGHT) {
                    slope_y_coord =
                        PX_TO_SUBPX(TILE_TO_PX(tile_y) + (8 - x_offset) -
                                    PLAYER.bounds.bottom) -
                        1;
                } else if ((col & COLLISION_SLOPE) ==
                           COLLISION_SLOPE_225_RIGHT_BOT) {
                    slope_y_coord =
                        PX_TO_SUBPX(TILE_TO_PX(tile_y) + (8 - DIV_2(x_offset)) -
                                    PLAYER.bounds.bottom) -
                        1;
                } else if ((col & COLLISION_SLOPE) ==
                           COLLISION_SLOPE_225_RIGHT_TOP) {
                    slope_y_coord =
                        PX_TO_SUBPX(TILE_TO_PX(tile_y) + (4 - DIV_2(x_offset)) -
                                    PLAYER.bounds.bottom) -
                        1;
                }
                    ...
                    slope_y_coord =
                        PX_TO_SUBPX(TILE_TO_PX(tile_y) +
                                    (x_offset)-PLAYER.bounds.bottom) -
                        1;
                } else if ((col & COLLISION_SLOPE) ==
                           COLLISION_SLOPE_225_LEFT_BOT) {
                    slope_y_coord =
                        PX_TO_SUBPX(TILE_TO_PX(tile_y) + DIV_2(x_offset) -
                                    PLAYER.bounds.bottom + 4) -
                        1;
                } else if ((col & COLLISION_SLOPE) ==
                           COLLISION_SLOPE_225_LEFT_TOP) {
                    slope_y_coord =
                        PX_TO_SUBPX(TILE_TO_PX(tile_y) + DIV_2(x_offset) -
                                    PLAYER.bounds.bottom) -
                        1;
            ...
            if (slope_y_coord) {
                ...
                UBYTE slope_top_tile_y =
                    PX_TO_TILE(SUBPX_TO_PX(slope_y_coord) + PLAYER.bounds.top);
                while (tile_start != tile_end) {
                    if (tile_at(tile_start, slope_top_tile_y) &
                        COLLISION_BOTTOM) {
                        pl_vel_y = 0;
                        pl_vel_x = 0;
                        PLAYER.pos.y = old_y;
        ...
        if (nocollide == 0) {
            ...
            while (tile_start != tile_end) {
                ...
                land:
                    new_y =
                        PX_TO_SUBPX(TILE_TO_PX(tile_y) - PLAYER.bounds.bottom) -
                        1;
                    actor_attached =
                        FALSE;  // Detach when MP moves through a solid tile.
            ...
            if (tile_at(tile_start, tile_y) & COLLISION_BOTTOM) {
                ...
                if (actor_attached) {
                    ...
                    if (last_actor->bounds.top > 0) {
                        temp_y +=
                            last_actor->bounds.top + last_actor->bounds.bottom
                            << 5;
                    }
                    new_y = temp_y;
                }
    ...
    if (hit_actor != NULL && hit_actor->collision_group) {
        ...
        if (hit_actor->collision_group == plat_solid_group) {
            if (!actor_attached || hit_actor != last_actor) {
                if (temp_y < (hit_actor->pos.y +
                              PX_TO_SUBPX(hit_actor->bounds.top)) &&
                    pl_vel_y >= 0) {
                    // Attach to MP
                    last_actor = hit_actor;
                    mp_last_x = hit_actor->pos.x;
                    mp_last_y = hit_actor->pos.y;
                    PLAYER.pos.y = hit_actor->pos.y +
                                   PX_TO_SUBPX(hit_actor->bounds.top) -
                    ...
                    // PLAYER bounds top seems to be 0 and counting down...
                           ...
                           hit_actor->pos.y +
                               PX_TO_SUBPX(hit_actor->bounds.bottom)) {
                    deltaY += (hit_actor->pos.y - PLAYER.pos.y) +
                              PX_TO_SUBPX(-PLAYER.bounds.top +
                                          hit_actor->bounds.bottom) +
                              32;
                    pl_vel_y = plat_grav;
                    ...
                    deltaX = (hit_actor->pos.x - PLAYER.pos.x) +
                             PX_TO_SUBPX(-PLAYER.bounds.left +
                                         hit_actor->bounds.right) +
                             16;
                    col = -1;
                    last_wall = -1;
                    wc_val = plat_coyote_max + 1;
        ...
        if (hit_actor && !hit_actor->collision_group &&
            hit_actor->script.bank) {
            script_execute(hit_actor->script.bank, hit_actor->script.ptr, 0, 1,
                           0);
        }
    }
}
...
gotoSwitch2:
    // SWITCH for Animation and State
    // Change==========================================================================
    switch (plat_state) {
        case FALL_STATE: {
            // ANIMATION--------------------------------------------------------------------------------------------------
        ...
        case JUMP_STATE: {
            ...
            // hits roof, c) player stops pressing, d)jump frames run out. JUMP
            // -> WALL check
            wall_check();
...
gotoTriggerCol:
    // FUNCTION TRIGGERS
    trigger_activate_at_intersection(&PLAYER.bounds, &PLAYER.pos,
                                     INPUT_UP_PRESSED);
...
gotoCounters:
    ...
    if (state_events[plat_state].script_addr != 0) {
        script_execute(state_events[plat_state].script_bank,
                       state_events[plat_state].script_addr, 0, 0);
    }
...
void ladder_check(void) BANKED {
    ...
    if (INPUT_UP || INPUT_DOWN) {
        ...
        if (IS_LADDER(tile_at(tile_x_mid, tile_y))) {
            PLAYER.pos.x = PX_TO_SUBPX(TILE_TO_PX(tile_x_mid) + 4 -
                                       (PLAYER.bounds.left + p_half_width));
...
void ladder_switch(void) BANKED {
    ...
    if (INPUT_UP) {
        ...
        que_state =
            FALL_STATE;  // Assume we're going to leave the ladder state,
        ...
        while (tile_start != tile_end) {
            if (tile_at(tile_x_mid - 1, tile_start) & COLLISION_RIGHT) {
                que_state =
                    LADDER_STATE;  // If there is a wall, stay on the ladder.
                break;
            }
            tile_start++;
        }
...
void dash_init_switch(void) BANKED {
    ...
    if (plat_dash_through == 3 && plat_dash_momentum < 2) {
        ...
        if (PLAYER.dir == DIR_RIGHT) {
            ...
            if (PLAYER.pos.x + PX_TO_SUBPX(PLAYER.bounds.right) +
                    (dash_dist * (plat_dash_frames)) >
                ...
                UBYTE tile_xr =
                    PX_TO_TILE(SUBPX_TO_PX(new_x) + PLAYER.bounds.right) + 1;
                UBYTE tile_xl =
                    PX_TO_TILE(SUBPX_TO_PX(new_x) + PLAYER.bounds.left);
                while (tile_xl !=
                       tile_xr) {  // This checks all the tiles between the left
                                   // bounds and the right bounds
                    while (tile_start !=
                           tile_end) {  // This checks all the tiles that the
                    ...
                    tile_start = PX_TO_TILE(
                        SUBPX_TO_PX(PLAYER.pos.y) +
                        PLAYER.bounds.top);  // Reset the height after each loop
                    tile_xl++;
                }
            }
        } else if (PLAYER.dir == DIR_LEFT) {
            // Don't dash off the screen to the left
            if (PLAYER.pos.x <= ((dash_dist * plat_dash_frames) +
                                 PX_TO_SUBPX(PLAYER.bounds.left)) +
                                    PX_TO_SUBPX(8)) {
                dash_end_clear =
                    false;  // To get around unsigned position, test if the
